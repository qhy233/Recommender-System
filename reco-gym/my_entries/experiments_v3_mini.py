...
print(f"\n配置: {NUM_PRODUCTS}物品, K={K}, σ={SIGMA}")
...
class EGreedyAgent(Agent):
    def __init__(self, config, eps=0.1):
        super().__init__(config)
        self.eps = eps
...
agents = [
    ('Popularity', PopAgent, {}),
    ('ε-greedy(0.1)', EGreedyAgent, {'eps': 0.1}),
    ('UCB(2.0)', UCBAgent, {'c': 2.0}),
    ('Thompson', TSAgent, {}),
]
...
ax.set_ylabel('CTR')
ax.set_title(f'复杂环境对比 ({NUM_PRODUCTS}物品, K={K})')
